...
// Custom HTTPS redirection that skips /metrics endpoint
app.Use(async (context, next) =>
{
    // Skip HTTPS redirection for /metrics endpoint (for Prometheus scraping)
    ...
    // Apply HTTPS redirection for all other endpoints
    if (!context.Request.IsHttps && context.Request.Host.Host != "host.docker.internal")
    {
